var taobao = function() {
  ...
  self.getShop = function() {
    ...
    function getShopId() {
      ...
      if (seller_id) {
        ...
        seller_id = $(rules.info.TAOBAO.shop.id_data_sellerid).attr(
          "data-sellerid"
        );
      }
  ...
  self.getProduct = function() {
    ...
    prod.getProductPrice = function() {
      var price = $(rules.info.TAOBAO.product.promoPrice)
        .text()
        .match(/[0-9,.]+/g);
      if (!price || !price.length) {
        price = $(rules.info.TAOBAO.product.basePrice)
          .text()
          .match(/[0-9,.]+/g);
      }
      for (var i in price) {
    ...
    prod.getProductImages = function() {
      var image = $(rules.info.TAOBAO.product.image)
        .attr("src")
    ...
    prod.updateView = function() {
      ...
      if (Array.isArray(price) && price.length > 1) {
        priceVnd =
          tool.convertToVND(price[0]) +
          "đ - " +
          tool.convertToVND(price[1]) +
          "đ";
      } else {
    ...
    prod.init = function() {
      ...
      if (product.itemPriceNDT && product.itemPriceNDT.length > 1) {
        product.itemPrice =
          "từ " +
          tool.convertToVND(product.itemPriceNDT[0]) +
          " đ đến " +
          tool.convertToVND(product.itemPriceNDT[1]);
      }
      // check first run
      if (!self.first) {
  ...
  self.renderView = function(product) {
    ...
    var priceVnd = "";
    if (
      Array.isArray(product.itemPriceNDT) &&
      product.itemPriceNDT.length > 1
    ) {
      priceVnd =
        tool.convertToVND(product.itemPriceNDT[0]) +
        "đ - " +
